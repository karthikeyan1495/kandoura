...
public class PendingApprovalTabstatuschanges extends java.util.Observable {

    Activity activity;
    OrderList orderList;
    MyProgressDialog myProgressDialog;
    public PendingApprovalTabstatuschanges(Activity activity, OrderList orderList) {
    ...
    public void OnClickpendingstatus(View view){

        final PopupMenu popupMenu = new PopupMenu(activity, view);

        popupMenu.getMenuInflater().inflate(R.menu.pendingapporovaltab, popupMenu.getMenu());
    ...
    private void updateAPICall(String order_id, String accept) {

        if (InternetChecker.getInstance().isReachable()) {
            myProgressDialog.showDialog(activity);
            APICall api = APIConfiguration.getInstance().createService(APICall.class);
            Observable<Response<GeneralResponse>> observable = api.orderststuschange(order_id,accept , MySession.getInstance(activity).getUser().getToken());
    ...
    public void OnClickordersinfo(View view){
